...
//  val testScenarioFinal = scenario("Post").exec(PostRequest.storeScn, PostRequest.fetchScn)
  ...
  val testScenarios = List(postAndGetInSeq.inject(rampUsers(5) over (1 minutes)))
    ...
    val prodLikeScenarios = List(randomPostRequest.inject(splitUsers(150) into (rampUsers(2) over (3 seconds)) separatedBy (4 seconds)), //atOnceUser(40)//rampUsers(4) over (2 minutes)),splitUsers(40) into (rampUsers(5) over (10 seconds)) separatedBy (10 seconds)
        randomGetRequest.inject( nothingFor(40 seconds), splitUsers(150) into (rampUsers(3) over (3 seconds)) separatedBy (4 seconds)))
