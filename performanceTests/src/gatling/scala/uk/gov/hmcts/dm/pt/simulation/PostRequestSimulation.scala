...
//  val testScenarioFinal = scenario("Post").exec(PostRequest.storeScn, PostRequest.fetchScn)
  ...
  val testScenarios = List(postAndGetInSeq.inject(rampUsers(5) over (1 minutes)))
    ...
    val prodLikeScenarios = List(randomPostRequest.inject(splitUsers(50) into (rampUsers(1) over (1 seconds)) separatedBy (1 seconds)), //atOnceUser(40)//rampUsers(4) over (2 minutes)),splitUsers(40) into (rampUsers(5) over (10 seconds)) separatedBy (10 seconds)
        randomGetRequest.inject( nothingFor(2 seconds), splitUsers(50) into (rampUsers(1) over (1 seconds)) separatedBy (1 seconds)))
