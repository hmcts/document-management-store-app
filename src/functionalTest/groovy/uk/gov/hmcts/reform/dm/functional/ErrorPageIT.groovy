...
import uk.gov.hmcts.reform.dm.functional.utilities.V1MimeTypes
...
class ErrorPageIT extends BaseIT {
    ...
    void "EP2 As an authenticated user trying to access an unknown document, receive JSON error page with 404"() {
        ...
        givenRequest(CITIZEN)
            .accept("application/vnd.uk.gov.hmcts.reform.dm.document-collection.v1+hal+json,application/json;charset=UTF-8")
            .expect()
                .contentType(ContentType.JSON)
                .statusCode(404)
            .when()
