...
public class FileContentVerifierTests {
    ...
    public void testUploadTikaDetectionFailure() throws Exception {
        MultipartFile file = new MockMultipartFile("files", "filename.txt",
                "text/plain", getClass().getClassLoader().getResourceAsStream(EXAMPLE_PDF_FILE));
    ...
    public void testUploadMimeTypeAllowedButDoesNotMatchActualType() throws Exception {
        MultipartFile file = new MockMultipartFile("files", "filename.txt", "tex",
                getClass().getClassLoader().getResourceAsStream(EXAMPLE_PDF_FILE));
        assertFalse(fileContentVerifier.verifyContentType(file));
    }
