...
public class RemoveSpringBatchHistoryTasklet implements Tasklet {
    ...
    private static final String SQL_DELETE_BATCH_JOB_EXECUTION = "DELETE FROM %PREFIX%JOB_EXECUTION WHERE " +
        " JOB_EXECUTION_ID IN (SELECT JOB_EXECUTION_ID FROM %PREFIX%JOB_EXECUTION WHERE CREATE_TIME < ? LIMIT 10000)";
